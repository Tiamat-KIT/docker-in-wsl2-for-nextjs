...
# Ayatana AppIndicatorライブラリのインストール
RUN apt-get install -y --no-install-recommends \
    libayatana-appindicator3-dev

# librsvg2のインストール
RUN apt-get install -y --no-install-recommends \
    librsvg2-dev  # librsvg2をインストール
...
RUN mkdir -p /opt/android-ndk && \
    curl -o /tmp/ndk.zip https://dl.google.com/android/repository/android-ndk-r23-linux-x86_64.zip && \
